...
deepspeed --include localhost:0,1,2,3,4,5,6,7 --master_port=30000 ${CUR_DIR}/src/train/train_mem_flash.py \
    --deepspeed ${CUR_DIR}/scripts/zero3.json \
    --model_name_or_path ${CUR_DIR}/playground/model/vicuna-7b-v1.5 \
    --version v1 \
    --data_path ${CUR_DIR}/playground/data/llava_v1_5_mix665k.json \
    --image_folder ${CUR_DIR}/playground/data \
    --vision_tower  ${CUR_DIR}/playground/model/clip-vit-large-patch14-336 \
    --pretrain_mm_mlp_align ${CUR_DIR}/playground/model/${PT_OUTPUT}/${BIN_NAME} \
    ...
    --bf16 True \
    --output_dir ${CUR_DIR}/playground/model/${FT_OUTPUT} \
    --num_train_epochs 1 \
